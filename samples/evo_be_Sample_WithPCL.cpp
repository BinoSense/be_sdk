	...
	while (!viewer->wasStopped(10))
	{
		...
		if (!be_fsm.beData.isMovingFastly)
		{
			if (be_fsm.beData.frame_id != beDataId_last)
			{
				...
				//获取转到0点的点云（注意PCL显示需要用删了INFINITY的无序点云）
				int point_count;
				float* p_pointcloud = vrht.retrievePointCloudZeroPoint(evo::depth::POINT_CLOUD_TYPE_POINT_CLOUD_UNORGANIZED_XYZBGRA, point_count, false);
				//转为PCL的数据格式，并且将单位从mm改为m
				cloud->clear();
				for (int m = 0; m < point_count; m++) {
					pcl::PointXYZRGBA p;
					p.x = p_pointcloud[m * 4] / 1000.0f;			//X
					p.y = p_pointcloud[m * 4 + 1] / 1000.0f;		//Y
					p.z = p_pointcloud[m * 4 + 2] / 1000.0f;		//Z
					int *color_ptr = (int *)p_pointcloud + m * 4 + 3;	//B-G-R-A
					p.rgba = *color_ptr;
