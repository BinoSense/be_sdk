#ifndef EVO_BEVIRTUALRECTIFYPIPELINE_HITNET_ONNX_H
#define EVO_BEVIRTUALRECTIFYPIPELINE_HITNET_ONNX_H

#include "evo_tinycgl_global_define.h"

#if defined(EVO_ENV_WINDOWS) && !defined(EVO_BEVIRTUALRECTIFYPIPELINE_HITNET_ONNX_EXPORTS)
#define EVO_BEVIRTUALRECTIFYPIPELINE_HITNET_ONNX_API EVO_WINDOWS_DLLIMPORT
#else
#define EVO_BEVIRTUALRECTIFYPIPELINE_HITNET_ONNX_API EVO_API
#endif
...
namespace evo
{
    namespace depth
    {
        class BEVirtualRectifyPipeline_Hitnet_OnnxVars;
        class EVO_BEVIRTUALRECTIFYPIPELINE_HITNET_ONNX_API BEVirtualRectifyPipeline_Hitnet_Onnx
        {
        public:
            BEVirtualRectifyPipeline_Hitnet_Onnx();
            ~BEVirtualRectifyPipeline_Hitnet_Onnx();
            ...
            //因为有平移，所以转回的结果只有点云，没有图片（需要渲染颜色的话，配合使用左图转回原角度的矫正后的图像）
            //获取左图转回0点的xyz距离
        ...
        private:
            BEVirtualRectifyPipeline_Hitnet_OnnxVars* vars;
        };
    }
}
