...
#if defined(EVO_BE_ENV_WINDOWS) && !defined(EVO_BE_COMMON_EXPORTS)
#define EVO_BE_COMMON_API EVO_BE_WINDOWS_DLLIMPORT
#else
#define EVO_BE_COMMON_API EVO_BE_API
#endif
...
namespace evo_be
{
    ...
    */
	struct BE_CalibData
	{
		...
		{
			for (int g = 0; g < 2; g++)
				this->eyes[g] = EyeTags[g];
			for (int g = 0; g < 3; g++)
				this->motors[g] = MotorTags[g];			
		}
     ...
     **************************************************/
	class EVO_BE_COMMON_API CBionicEyes
	{
	public:
        /**************************************************
         ...
         **************************************************/
		static CBionicEyes *create(std::string ipAddr,
                                    BE_Connect_Type type = enumdisConnect,
                                    BE_Connect_DataServerType dataServerType = enumDeviceServer_Only,
                                    BE_Data_TransmissionType dataTransmissionType = enumDataTransmission_ASPAP,
                                    void *logger_ptr = NULL);

        static CBionicEyes *create(std::string SrvipAddr,
                                   std::string devName,
...
//                                   void *logger_ptr = NULL);
         ...
         **************************************************/
		virtual std::string getBeDevice_Ip_str() = 0;
         ...
         **************************************************/
		virtual BE_CalibData getCalibrationInfo() = 0;
         ...
         * named path_T.
         **************************************************/
		virtual void saveBeData(bool startStop = 1, std::string folderPath = Be_Io_Path + "fastSave") = 0;
         ...
         **************************************************/
		virtual bool snapBeData(std::string description="", std::string folderPath = Be_Io_Path + "snapSave") = 0;
         ...
         * generate a new path named path_T.
         **************************************************/
        virtual bool recordBeData(BE_GeneralData &data, std::string description="", std::string folderPath = Be_Io_Path + "recordSave") = 0;
